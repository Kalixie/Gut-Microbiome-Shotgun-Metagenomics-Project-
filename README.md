...
Through the use of publicly available human gut microbiome sequencing data, this study aimed to investigate differences in gut microbial composition between individuals with vegan and omnivorous diets. Vegan diets are typically higher in fiber, whereas omnivorous diets contain more protein and fat (Łuszczki et al., 2023). These differences throughout long-term dietary patterns can alter the diversity and function of the gut microbiome (Zhang, 2022). Specific microbial taxa have been shown to correlate with human health outcomes, highlighting the importance of further research into the differences between specific diets for nutritional studies (Fackelmann et al., 2025). 

The data used in this study were collected from the National Center for Biotechnology Information (NCBI). All collected samples were from subjects based in Turin, Italy, for consistency, with human reads already removed from microbiome data. Three omnivore samples (SRR8146971, SRR8146969, SRR8146936) and three vegan samples (SRR8146978, SRR8146977, SRR8146974) were used for analysis.

The data collected was used to perform a shotgun metagenomic analysis. Taxonomic classification was performed using Kraken2, a k-mer-based classifier chosen for its speed and accuracy. Kraken is widely used in taxonomic classification studies and has a wide availability of datasets for analysis. Bracken was used alongside Kraken2 to reestimate taxon abundances, correcting for biases in read assignment, as Kraken2 can occasionally misestimate abundance for related taxa (Lu et al., 2017). Previous studies have found Kracken2 and Bracken to be efficient for microbial analysis in comparison to alternatives such as Diamond and Megan (Xu et al., 2023)

Downstream analysis was conducted in R using the phyloseq package, which allows for abundance data to be visualized along with metadata through the use of vegan and ggplot2 (McMurdie and Holmes, 2013). Alpha and beta diversity metrics were used to assess sample variation, while differential abundance testing was performed using ANCOMBC2, a method that accounts for compositionality and taxon bias in microbiome data (Lin and Peddada, 2023). ANCOMBC2 was used because microbiome data is compositional, which standard statistical methods such as ANOVA do not consider (Lin and Peddada, 2023).
...
Rarefaction curves for all six samples reached a plateau, which indicated that sequencing depth was sufficient for this data and therefore suggested that the majority of microbial diversity present in each sample was captured. This confirms that the data contains enough information for further analysis.
...
When datasets were compared at the phylum level for vegan and omnivore groups, the microbial composition observed was similar (Figure 2). All six samples were largely composed of members of the phylum Bacteroidota, with Bacillota also consistently represented in the relative abundance. The level of Verrucomicrobiota in sample SRR8146974 was notably higher than all other samples, suggesting a potential outlier within the data. Consistent separation was not seen between diets at this taxonomic level. 
...
Analysis performed at the Genus level showcased more distinct differences between dietary groups (Figure 3). Omnivorous samples showed consistently high relative abundance of Segatella in all three samples, whereas vegan samples contained a higher abundance of Alistipes in one sample and Bacteroides in two samples (Figure 3). To confirm that these patterns were not biased, the genus-level composition was also visualized using the top 10 taxa within each individual sample, which produced similar results and supported the differences observed between dietary groups at this taxonomic level (Supplementary Figure: figures/RA4Genus10persample.png).
...
Alpha diversity metrics showed variability within both dietary groups, with higher diversity in the vegan samples in comparison to the omnivore groups. Vegan samples exhibited slightly higher diversity values within both Shannon and Simpson indices. Omnivore samples exhibited lower diversity in two of the three samples observed.
...
Beta diversity analyses revealed partial separation between dietary groups (Figure 5). Bray-Curtis PCoA showed some distinction between vegan and omnivorous samples, although one vegan sample appeared as an outlier. Bray-Curtis NMDS distances showed weaker clustering and greater overlap between groups (Supplementary Figure: figures/BrayNMDS.png). Jaccard PCoA demonstrated moderate clustering by diet, with one vegan sample deviating from the group. 
...
PERMANOVA analysis using Bray-Curtis distances showcased that diet explained approximately 35.8% of the variation in microbial composition, although this effect was not statistically significant. Differential abundance testing using ANCOMBC2 identified no genera that were significantly different between vegan and omnivorous groups after multiple testing correction (Figure 6). 
